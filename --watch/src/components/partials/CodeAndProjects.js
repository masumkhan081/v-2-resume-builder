...
export default function CodeAndProjects() {
  const { state, updateState, removeProject } = useContext(page3Context);
  ...
  function setForEdit(i) {
    titleRef.current.value = state.projects[i].title;
    descrRef.current.value = state.projects[i].description;
    linkRef.current.value = state.projects[i].srcLink;
    setAct({ ...act, type: "edit", actOn: i });
  }
  function deleteItem(ind) {
    updateState({
      ...state,
      projects: state.projects.filter((project, itemInd) => {
        return ind !== itemInd;
      }),
    });
  }
  ...
  function handleSubmit(e) {
    ...
    if (title.length < 3) {
      ...
      if (act.type == "add") {
        updateState({
          ...state,
          projects: [...state.projects, { title, description, srcLink }],
        });
      }
      if (act.type == "edit") {
        updateState({
          ...state,
          projects: state.projects.map((item, ind) => {
            if (act.actOn == ind) {
