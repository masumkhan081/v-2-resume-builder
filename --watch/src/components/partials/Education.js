...
export default function Education() {
  ...
  function setForEdit(i) {
    degRef.current.value = state.educations[i].degreeName;
    instRef.current.value = state.educations[i].institution;
    passYearRef.current.value = state.educations[i].passYear;
    setAct({ ...act, type: "edit", actOn: i });
  }
  function deleteItem(ind) {
    updateState({
      ...state,
      educations: state.educations.filter((item, itemInd) => {
        return ind !== itemInd;
      }),
    });
  }
  ...
  function handleSubmit(e) {
    ...
    if (institut.length < 3) {
      ...
      if (act.type == "edit") {
        ...
        updateState({
          ...state,
          educations: state.educations.map((item, ind) => {
            if (act.actOn == ind) {
              return {
                degreeName: degreeN,
  ...
  return (
    <div className="col-md-5 " style={{ height: "80vh" }}>
      ...
      <span>{JSON.stringify(state.educations)}</span>
    </div>
  );
}
