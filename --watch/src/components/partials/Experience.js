...
import { page2Context } from "../pages/Page2";
...
export default function Experience() {
  const { state, updateState } = useContext(page2Context);
  ...
  function setForEdit(i) {
    jobTitleRef.current.value = state.experiences[i].jobTitle;
    employerRef.current.value = state.experiences[i].employer;
    //
    let start = state.experiences[i].duration.split("to")[0];
    startMRef.current.value = start.split("-")[0];
    startYearRef.current.value = start.split("-")[1];
    //
    let end = state.experiences[i].duration.split("to")[1];
    if (end == "Present") {
      setCurrWorking(true);
    } else {
  ...
  function deleteItem(ind) {
    updateState({
      ...state,
      experiences: state.experiences.filter((item, itemInd) => {
        return ind !== itemInd;
      }),
    });
  }
  ...
  function handleSubmit(e) {
    ...
    if (jobTitle.length <= 1 && employer.length <= 1) {
      ...
      if (act.type == "add") {
        updateState({
          ...state,
          experiences: [...state.experiences, { jobTitle, employer, duration }],
        });
      }
      if (act.type == "edit") {
        updateState({
          ...state,
          experiences: state.experiences.map((item, ind) => {
            if (act.actOn == ind) {
              return {
                jobTitle,
                employer,
  ...
  return (
    <div className="col-md-7  " style={{ height: "80vh" }}>
      ...
      <span>{JSON.stringify(state.experiences)}</span>
    </div>
  );
}
